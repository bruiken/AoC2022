...
def dfs(time_left, current_valve, closed_valves, elephant):
    best = 0
    for v in closed_valves:
        ...
        if dist < time_left:
            ...
            rest_pressure = dfs(remaining_mins, v, closed_valves - {v}, elephant)
            if pressure_released + rest_pressure > best:
                best = pressure_released + rest_pressure
    if elephant:
        elephant_score = dfs(minutes, 'AA', closed_valves, False)
        if elephant_score > best:
            best = elephant_score
    return best

print(dfs(minutes, 'AA', frozenset(flows), True))
